...
- It prohibits kernel memory recycling threads running on all cores, however, kswapd one of the recycling threads is fixed on the small cores but to avoid congestion of the cores that are interacting it is done through parallelism, which in addition to allowing better use of reclaim, reduces energy consumption

...
- Includes miscellaneous optimizations. Such as optimized PPR availability on Snapdragon devices, allowing a secondary reclaim with a higher cache hit chance than the stock default, an option that some users might like to use if they rely on PPR. Avoid unnecessary memory reservations. Fixes for certain subsystems that were causing issues and regressions. Pinning of essential Android libs, to reduce Jank that occurs when system libs are swapped unintentionally. In addition to including adjshield, a way for you to protect your favorite apps (or whatever you want) from the evil lmk that kills them, working for all forms of lmk

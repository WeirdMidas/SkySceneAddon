...
A memory management module for Android devices, it also modernizes the Android operating system itself. It's entirely focused on modernizing the way Android manages its resources, uses its technologies, and manages memory, with a primary focus on energy efficiency and advanced multitasking. SkyScene is based on the idea of ​​this module here: https://developer.aliyun.com/article/1230689.

## ⭐ Features

- Pure memory management optimization, it does not contain any other placebo or anything that current Androids no longer use or are deprecated or outdated. In addition to focusing on memory management, it also focuses on modernizing the Android devices that use them, allowing them to use more efficient technologies for their specific architectures, allowing users to extract the maximum potential in both efficiency and performance and stability from their respective SOCs
...
- Instead of suffering from memory-outage panics, use oom_killer, but not before compacting memory to avoid killing user processes. This reduces the chances of OOM and allows for better survival of background apps, favoring a higher level of stability in memory management than stock parameters can offer. However, there are still limits if memory pressure is so high that compression and oom_kiler fail to save in these situations, forcing the kernel to panic. However, this only happens on low-mem devices and is fixed by integrating the proactive swapping strategy
- ART/JIT optimizations and improvements for better performance, efficiency, and CPU and RAM usage for app compilation, whether in the background, updating the system, or in the foreground. Also including a GC overhaul, choosing the best garbage collector to avoid memory fragmentation and allocation errors depending on your kernel and compatibility level
- A comprehensive set of I/O improvements and optimizations for UFS and eMMC storage devices. These improvements improve I/O bursting, I/O request grouping, and read-ahead for dynamic partitions, reducing power consumption and memory waste, as well as improving latency and throughput, significantly reducing I/O stalls
...
- Specific SOCs, manufacturers, and ROMs have their own optimizations and improvements for energy efficiency, GPU and CPU performance, fluidity, memory management, and I/O. These include optional optimizations that the user can choose to use or not. These optimizations are intended to improve how the system and kernel handle I/O, CPU, and memory stalls, favoring greater multitasking, fluidity, and energy efficiency simultaneously
- The module provides customizable parameters, among them variable virtual memory sizes such as 0g up to 6g ZRAM, 0g to 3g Swapfile and 0g to 8g Write-back, also customizable compression and I/O algorithm selection. Many parameters can be modified in the module panel by yourself, and will take effect after reboot. Pay attention to the comments
...
- Inclusion of intelligent caching techniques in several areas of the system (GPU, CPU, I/O, etc.), allowing for improved energy efficiency at the expense of slightly higher memory consumption, which will be completely offset by the optimizations we applied to ZRAM and VM. Also: no prefetch! If we use prefetch, it will only be to improve performance for extremely linear tasks. Otherwise, prefetch will never be included, and some will even be disabled due to its wasteful guessing
...
- Includes miscellaneous optimizations. Such as optimized PPR availability on Snapdragon devices, allowing a secondary reclaim with a higher cache hit chance than the stock default, an option that some users might like to use if they rely on PPR. Zygote optimizations for better efficiency and app launch on devices that do not have such optimizations. Use a batching technique between various subsystems, allowing you to wake up CPU less often by grouping memory management tasks in an on an on an on -lot. Reducing the unnecessary wakeup of these subsystems. Moderns GPU technologies for older devices, where depending on their compatibility, GPU performance and efficiency improves considerably. Avoid unnecessary memory reservations. Fixes for certain subsystems that were causing issues and regressions. Pinning of essential Android libs, to reduce Jank that occurs when system libs are swapped unintentionally. In addition to including adjshield, a way for you to protect your favorite apps (or whatever you want) from the evil lmk that kills them, working for all forms of lmk
...
- After all memory management optimizations, a general cleanup and reorganization is applied, designed solely to eliminate as many overflow cache situations as possible and allow the user to have as much of their memory available for use as possible, without fragmentation, overflow cache, etc., just maximum efficiency. Then, lmkd is reinitialized, allowing lmkd to write the entire system state to memory, maximizing lmkd's accuracy to considerable levels
...
Q. Why did the module change from memory management to a modernization module?   
A. SkyScene decided to go a step further than simply being a memory management module, now focusing on optimizing memory management and also providing modernization across several different devices. This allows older devices to **try** using current technologies to get even more out of their SOC, drastically reducing all possible I/O, MEM, and CPU stalls by enabling better integration with the processor through newer, better technologies.

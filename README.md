...
- Additional compression every 60 seconds of apps that are running in the background. It's not ZRAM, it's a memory management technique called CompAction that compresses open apps when they're not being used, this generally reduces their memory usage by ~10-30% depending on their complexity, and they can also be sent to ZRAM, further reducing their memory usage in an absurd way

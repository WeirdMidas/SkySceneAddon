...
- Addition of a second third-party module-app, Cirno! A third-party module that allows the user system the ability to use Freeze Cgroup V2, allowing the system to dynamically freeze processes based on whether they should not be awake for a long time, or whether they are not essential processes such as messengers, etc. However, this adjustment is only for kernel 4.19 (on devices with Qualcomm processors) and kernel 5.4 (for other processors) or higher in both cases, and requires Android 12 or higher to be able to use it

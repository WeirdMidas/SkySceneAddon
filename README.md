...
- Avoid premature reclaim by using watermark_scale_factor consistent with the amount of device memory, which makes reclaim less aggressive and more efficient, causing the user to have an increase of up to ~2x in the number of open processes compared to before, and still with the same or slightly lower memory consumption.

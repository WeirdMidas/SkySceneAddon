...
- Dynamic Swapping focuses on one thing: Scalability, meaning that the swap in general is scalable based on user demand, with it changing three main parameters in a "smooth" manner that allows transitions between different pressures. Allowing kswapd to adapt to the system pressure, becoming less or more aggressive based on that. This in general allows the system to easily reclaim memory under different situations, maintaining stability over swap usage and reducing overall power usage

...
- Introduce hybrid swap! A virtual memory management technique that combines swapfile + zram + ppr, allowing swapping costs to be reduced by up to 27% and still increasing effective memory by 17% even with 1gb of swapfile, It also exponentially reduces writes to storage by allowing a more beneficial swapfile reclaim. As such, it can only be used on devices with PPR (per-process reclaim) which are only found on phones with qualcomm processors. The method by which this reclaim is done is by using the swapfile as a "backup", following a strategy similar to macOS, where the swapfile is like a table where the user's books (data) are stored up to a certain limit. Hybrid swap follows this same strategy by using PPR, to allocate non-essential process data in the swapfile instead of sending it to ZRAM directly, allowing ZRAM to be relaxed and to compress only essential data. Based on this, the reclaim success rate increases and is above 80% in most situations even under memory pressure, increasing page-cache retention to very high levels. It's not a higher level of success rate because PPR can be aggressive, but it makes up for it with the effective memory being technically +17% larger than the original set swapfile size (example: 1gb swapfile would be equivalent to 1.19gb swapfile in efficiency)

...
- Customizable ZRAM size, compression algorithm, whether or not to use ZRAM dedup and addition of ZRAM Writeback (needs kernel support), with the ZRAM size ranging from 0G to 6G and the Writeback size fixed at 3G. And the module will try to have the lowest compression rate of 2.8x, with the attempt to achieve average compression equal to or above the ideal rate of modern Androids of 3.1x
